function [g,b] = gradientFuncHingeBinary_L1(theta,tolerance,otherParas)
...
regPara = 100*tolerance; % L1 relulization parameter
...
for i = 1:nUnqSample
    ...
    if (slope(i) >= 2 - tolerance) && (slope(i) <= 2 + tolerance)
        b{1} = [b{1}; i];
        b{2} = [b{2} [featureDifference(:,i);-2]];  %  A colume vector which is the norm vector of the hyperplane that the point is on, with the b in wx+b = 0 as the last element.
    elseif (slope(i) >= -2 - tolerance) && (slope(i) <= -2 + tolerance)
        b{1} = [b{1}; i];
        b{2} = [b{2} [featureDifference(:,i);2]];
    end
end

for l = 1:nFeature % Finding boundaries on theta(i) = 0 which is introduce by regulazation.
    if norm(theta(l),1) < tolerance
        b{1} = [b{1}; nUnqSample + l]; % Using nUnqSample+l to represent that the hyperplane is defined as the l th langrange parameter = 0
        normV = zeros(nFeature+1,1); % There is an additional 0 at the end as the b in wx+b = 0 form.
        normV(l) = 1;
        b{2} = [b{2} normV];
    end
end
...
g = g - statistics * freq + regPara * theta./abs(theta); % Using L1 regulazation
