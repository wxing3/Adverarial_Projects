function g = gradientFuncHinge(theta,otherParas)
...
%   concatenate all the vectors.
%   b - cell(2,1): Information about whether a point is on the boundarys,
%   which includes:
%       b{1}: The index of sample which define the boundary with a sign. If
%       the sign is + then the plane is has "xxx = 2" form, other wise it
%       has "xxx = -2" form.
%       b{2}: nBoundaries * sum(nFeatureDimension): A matrix in which each
