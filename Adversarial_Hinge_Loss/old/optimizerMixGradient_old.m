function [minParas,minF,k] = optimizerMixGradient_old(varargin)
...
previousG = zeros(1,nUnknownParas); % Previous greadient function value
...
previousParas = zeros(1,nUnknownParas); % Previous unknown parameters value
grandParas = previousParas; % Unknown parameters value 2 steps ago.
previousD = zeros(1,nUnknownParas); % Direction (used to find next point) that previous point has.
            ...
            case 1 % Normal subgradient method
                ...
                if abs(g * d') <= tolerance % If current gradient is perpendicular to current direction,
                    if nBoundaries > 0 %If current point is on a hyperplanetry, then constrain the movement on the hyperplane directly, to prevent step into this region again.
                        ...
                        for j = 1:nBoundaries
                            if hyperPlanes{1}(j) > 0
                                const = -2;
                            else
                                const = 2;
                            end
                            hyperPlaneList.push([hyperPlanes{2}(:,j); const]);
                        end
    ...
    switch updateFlag
        ...
        case 3 % If binary search cannot find better result, we will try to constrain the problem on one of the hyperplanes.
            if isPlaneConstrained
                ...
                normV = hyperPlane(1:length(hyperPlane)-1); % Normal vector of the hyperplane, a colume vector
