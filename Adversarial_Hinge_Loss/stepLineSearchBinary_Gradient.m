function x = stepLineSearchBinary_Gradient(varargin)
...
if isEstimateAll
    g1 = gradFunc(varargin{2}(:,1),tolerance); % In binary search, the tolerance of boundary should be 0, so that the minimal point can get the real minimal point, otherwise the minimal point will become one of theat * featureDif = 2 + tol and theat * featureDif = 2 - tol
    g2 = gradFunc(varargin{2}(:,2),tolerance);
else
    g1 = gradFunc(varargin{2}(:,1),tolerance,knownParas); % Actually we should have different tolerance on different hyperplanes, if initial points are on the same hyperplane, than we should have tolerance on these planes and 0 on other planes
    g2 = gradFunc(varargin{2}(:,2),tolerance,knownParas);
end
...
while norm(left - right) >= tol && k<=maxIter %This can be very slow if left or right is too small
    ...
    if isEstimateAll
        middleG = gradFunc(middle,tolerance);
    else
        middleG = gradFunc(middle,tolerance,knownParas);
    end
