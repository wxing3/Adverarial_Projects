function [score, theta, output] = learn(measure, learningFileName, learnInitialTheta, ...
    ...
    elseif strcmpi(measure, 'DCG')
        ...
        [nRow, nRawFeature] = size(rowFeatureMatrix);
...
%         nSample = nRow/extraPara;
        ...
        for j = 1:nRel_real
           ...
           trueTags(idx) = ones(sum(idx),1) * (j-1); % convert all tags to 0 to nRel-1 number tags
        end      
        
%         trueFeatureValues = zeros(nRel_real * nRawFeature,1); %Since the expectation on samples is actually summation, we can add the true features all together here
%         for j = 1:nRel_real
%             trueFeatureValues((j-1)*nRawFeature+1:j*nRawFeature) = rowFeatureMatrix' * (trueTags==(j-1)); 
%         end        
%         trueFeatureValues = trueFeatureValues / nSample;
%         
%         if nRel_real == 2
%             trueFeatureValues = trueFeatureValues(nRawFeature+1:end); % For binary case, label 0 cases have 0 as their features
%         end
        ...
        trueFeatureValues = zeros(nRel_real * nRawFeature,nSample); %Since the expectation on samples is actually summation, we can add the true features all together here
       
        startIdx = 1;
        for i = 1:nSample
            featureMatrix{i} = rowFeatureMatrix(startIdx:startIdx+nDocs(i)-1,:); % Reshape the feature for each sample of query, document pair
            sampleTrueTags = trueTags(startIdx:startIdx+nDocs(i)-1);
            startIdx = startIdx + nDocs(i);
            
            for j = 1:nRel_real
                trueFeatureValues((j-1)*nRawFeature+1:j*nRawFeature,i) = (featureMatrix{i})' * (sampleTrueTags==(j-1)) / nDocs(i);
            end
        ...
        trueFeatureValues = sum(trueFeatureValues,2) / nSample;
...
%         if length(learnInitialTheta)==1 && nRel == 2
%             theta = [theta;-theta]; % If relevance level is binary, expand LR initial value or feature length value to its twice 
%         end
    else
