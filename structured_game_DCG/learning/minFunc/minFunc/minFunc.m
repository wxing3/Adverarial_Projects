...
nUnconverge = 0;
unconvergeLimit = 1700;

if method < NEWTON
    [f,g,unconvergeFlag] = funObj(x,varargin{:});
    computeHessian = 0;
else
    [f,g,H,unconvergeFlag] = funObj(x,varargin{:});
    computeHessian = 1;
end
...
% Exit if inner problem do not converge for more than limited times.
if unconvergeFlag
    nUnconverge = nUnconverge + 1;
    if nUnconverge > unconvergeLimit
        exitflag = 0;
        msg = ['Inner optimizaitno failed to converge more than ' num2str(unconvergeLimit) ' times.'];
        if verbose
            fprintf('%s\n',msg);
        end
...
% Create the file to store the x got in each step
%dataName = strrep(varargin{3},'/','.');
%dataName = strrep(dataName,'.train.ndoc','');
%fprintf('%s,%s,%s,%d/n',varargin{3},dataName,varargin{4}.method,varargin{4}.parameter);
fileID = fopen(['theta_', varargin{4}.method, '_', num2str(varargin{4}.parameter), '.log'] ,'w');
tic;
...
for i = 1:maxIter
    ...
    if LS_type == 0 % Use Armijo Bactracking
        ...
        if computeHessian
            [t,x,f,g,LSfunEvals,nUnconverge,H] = ArmijoBacktrack(x,t,d,f,fr,g,gtd,c1,LS_interp,LS_multi,progTol,debug,doPlot,LS_saveHessianComp,funObj,nUnconverge, unconvergeLimit,varargin{:});
        else
            [t,x,f,g,LSfunEvals,nUnconverge] = ArmijoBacktrack(x,t,d,f,fr,g,gtd,c1,LS_interp,LS_multi,progTol,debug,doPlot,1,funObj,nUnconverge, unconvergeLimit, varargin{:});
        end
    ...
    elseif LS_type == 1 % Find Point satisfying Wolfe conditions
        ...
        if computeHessian
            [t,f,g,LSfunEvals,nUnconverge,H] = WolfeLineSearch(x,t,d,f,g,gtd,c1,c2,LS_interp,LS_multi,25,progTol,debug,doPlot,LS_saveHessianComp,funObj,nUnconverge,unconvergeLimit,varargin{:});
        else
            [t,f,g,LSfunEvals,nUnconverge] = WolfeLineSearch(x,t,d,f,g,gtd,c1,c2,LS_interp,LS_multi,25,progTol,debug,doPlot,1,funObj,nUnconverge,unconvergeLimit,varargin{:});
        end
        ...
        if method >= NEWTON
            [f_new,g_new,H, unconvergeFlag] = funObj(x + t*d,varargin{:});
            funEvals = funEvals + 1;
    ...
    if verboseI
       ...
       % record theta, damp into theta.file
          fprintf(fileID, '%g\n', toc);
	     fprintf(fileID, '%g\n', f);
	        fprintf(fileID, '%s\n', sprintf('%g ', x)); 
