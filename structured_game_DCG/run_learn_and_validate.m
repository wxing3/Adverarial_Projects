...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
datasetName = 'Toy2';
kPercent = 0.1; % F1, Precision, Recall (case insensitidve)
measure = 'DCG';
...
%regularizationValues = [2^-7,2^-6,2^-5, 2^-4, 2^-3, 2^-2, 2^-1, 2^0, 2^1, 2^2, 2^3];
regularizationValues = [2^-4, 2^-3, 2^-2, 2^-1,1, 2];
%regularizationValues = [2^-5];
...
for i = 1:length(trainingFileNames)
    ...
    for regularizationValue = regularizationValues
        ...
        if strcmpi(measure, 'DCG')
            extraPara = trainingNDocFileName;
        else
            extraPara = kPercent;
        end
        %% training
        ...
        %% validation
        if strcmpi(measure, 'DCG')
            extraPara = validationNDocFileName;
        else
            extraPara = kPercent;
        end
        
%         param.testfilename = ['data/' validationFileName];
%         param.testndocfilename = ['data/' validationNDocFileName];
%         [NDCG, meanNDCG] =  testprediction(param, theta);
...
%         
        if strcmpi(measure, 'precision') || strcmpi(measure, 'recall') || strcmpi(measure, 'f1')
            ...
            if(result.maxProbMaximizerScore > bestScore)
                bestRegularizationValue = regularizationValue;
                bestScore = result.maxProbMaximizerScore;
            end
        elseif strcmpi(measure, 'DCG')
            ...
            if(result.avg_maximizerScore > bestScore)
                bestRegularizationValue = regularizationValue;
                bestScore = result.avg_quasiRobustMaximizerScore;
            end
            
%             fprintf('NDCG@1, NDCG@2, NDCG@3, NDCG@4, NDCG@5, NDCG@6, NDCG@7, NDCG@8, NDCG@9, NDCG@10, NDCG@ALL:\n\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\n', ...
%                 NDCG(1), NDCG(2), NDCG(3), NDCG(4), ...
%                 NDCG(5), NDCG(6), NDCG(7), NDCG(8), NDCG(9), ...
%                 NDCG(10), meanNDCG);
%             if(meanNDCG > bestScore)
%                 bestRegularizationValue = regularizationValue;
%                 bestScore = meanNDCG;
%             end
        end
        
        fprintf('The best meanNDCG is %.6f with regularization parameter %f.\n',bestScore,bestRegularizationValue);
        
        save([thetaFilePrefix regularization.method '_' num2str(regularization.parameter) '.NDCG.mat'],  'result');
        
%         save([thetaFilePrefix regularization.method '_' num2str(regularization.parameter) '.NDCG.mat'],  'meanNDCG', 'NDCG');

    end
    
%     fprintf('Best regularization value is %.6f\n',bestRegularizationValue);
%     fprintf('=========================================================\n');
end
